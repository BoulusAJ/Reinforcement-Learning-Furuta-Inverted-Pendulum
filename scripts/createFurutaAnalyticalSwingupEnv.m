function env = createFurutaAnalyticalSwingupEnv(cfg)
%CREATEFURUTAANALYTICALSWINGUPENV MATLAB-only swing-up environment.
%
% The agent runs at cfg.Agent.SampleTime. Current is held constant during
% that interval and the nonlinear mechanical plant is integrated using RK4
% substeps of cfg.MatlabEnvironment.IntegrationStep.
...
for idx = 1:cfg.MatlabEnvironment.Substeps
    x = localRk4Step(x, motorTorque, cfg.MatlabEnvironment.IntegrationStep, logged.Param);
end
...
function xNext = localRk4Step(x, torque, dt, param)
k1 = furutaAnalyticalStateDerivative(x, torque, param);
k2 = furutaAnalyticalStateDerivative(x + 0.5 * dt * k1, torque, param);
k3 = furutaAnalyticalStateDerivative(x + 0.5 * dt * k2, torque, param);
k4 = furutaAnalyticalStateDerivative(x + dt * k3, torque, param);
...
omega = min(max(x(3:4), -omegaClip), omegaClip);
omega = omega / cfg.Observation.OmegaScale;

obs = [sin(theta1Error); cos(theta1Error); ...
       sin(theta2Error); cos(theta2Error); ...
       omega; previousAction];
end
