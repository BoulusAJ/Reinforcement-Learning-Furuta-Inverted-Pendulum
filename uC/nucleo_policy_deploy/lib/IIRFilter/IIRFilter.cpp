...
void IIRFilter::differentiatingLowPass1TustinInit(const float fcut, const float Ts)
{
    filter.order = 1;
    differentiatingLowPass1TustinUpdate(fcut, Ts);
    resetDifferentingFilterToZero(0.0f);
}

void IIRFilter::differentiatingLowPass1TustinUpdate(const float fcut, const float Ts)
{
    const float Tf = 1.0f / (2.0f * M_PIf * fcut);
    const float a = 2.0f * Tf / Ts;
    const float k = 1.0f / (a + 1.0f);

    filter.A[1] = 0.0f;
    filter.B[0] = (2.0f / Ts) * k;
    filter.B[1] = -filter.B[0];
    filter.B[2] = 0.0f;
    filter.A[0] = (1.0f - a) * k;
}
