...
omega1ErrorAbsMax = max(abs(sig.omega1Error));
omega2ErrorAbsMax = max(abs(sig.omega2Error));
omegaAbsMax = max([omega1ErrorAbsMax, omega2ErrorAbsMax]);
...
settlingTimeTheta2Pct2 = computeSettlingTime(t, sig.theta2Error, getTheta2Pct2SettlingTol(evalCfg));
...
row = table( ...
    ...
    omega1ErrorAbsMax, omega2ErrorAbsMax, ...
    theta2IAE, theta2ISE, theta1IAE, ...
    settlingTimeTheta2, settlingTimeTheta2Pct2, settlingTimeBoth, ...
    ...
    'VariableNames', [ ...
        ...
        "MaxAbsOmega1Error", "MaxAbsOmega2Error", ...
        "Theta2IAE", "Theta2ISE", "Theta1IAE", ...
        "SettlingTimeTheta2", "SettlingTimeTheta2Pct2", "SettlingTimeBoth", ...
        "FinalTheta2MAE", "FinalTheta1MAE", "FinalTheta2Bias", "FinalTheta1Bias", "FinalTheta2MaxAbsError", ...
        "Theta2EndRMS", "Theta2EndOscRMS", "Theta2EndPeakToPeak", "Theta2EndOscFreqHz", ...
...
function tol = getTheta2Pct2SettlingTol(evalCfg)
if isfield(evalCfg, "SettlingTolTheta2Pct2")
    tol = evalCfg.SettlingTolTheta2Pct2;
else
    tol = 0.02 * pi;
end
end
