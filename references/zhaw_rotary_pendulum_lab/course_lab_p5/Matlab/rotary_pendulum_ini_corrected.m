...
% Velocity control loop has no big influence here
figure(44)
rlocus(L_phi2*T_om_mod), grid on
axis([-40 10 -20 20])

Kp_phi2 = 45.2; % Faster double real poles

% Why is the motor accelerating without additional feedback=
feedback(Kp_phi2 * C_phi2, G_phi2)
...
L_v = minreal(L_v); % (does not include Kp)
S_v = feedback(1, Kp_v * L_v);
